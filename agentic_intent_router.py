...
class StageZeroRouter:
    ...
    def classify_intent(self, user_query: str, persona: str = None, 
        ...
        "CRITICAL — CU ID ROUTING RULE:\n"
        "If user mentions a CU ID like CU_COMMUNITY_003, CU_ACU_001, CU_ALAMO_004:\n"
        "  → If query contains 'why', 'explain', 'how', 'what made', 'reason', "
        "'ranked', 'score for', 'tell me about', 'deep dive' → route to KNOWLEDGE_BASE\n"
        "  → If query contains 'find', 'run', 'process', 'identify', 'match' → route to MATCHING_ENGINE\n"
        "  → If query contains 'compare', 'benchmark', 'peer' → route to MARKET_DATA\n"
        "  The CU ID alone does NOT force MATCHING_ENGINE — the verb/intent determines the route.\n"
        ...
        Trigger this for ALL explanatory, conceptual, or "why/how/what" questions.

        This includes TWO sub-types:

        1. GENERAL METHODOLOGY (no prior run needed):
        "What is loan boxing?", "How do you calculate the LTD bonus?",
        "Explain risk-weighting", "What is the 701.22 requirement?",
        "How does the matching engine work?", "Explain the scoring logic"

        2. SESSION-SPECIFIC EXPLANATION (after engine has run):
        "Why is CU_COMMUNITY_003 ranked #1?",
        "Why was CU_ACU_001 ranked #2?",
        "Explain the score for CU_ALAMO_004",
        "What made CU_COMMUNITY_003 the top match?",
        "Explain the matching logic used above",
        "Why did this CU score higher than others?"

        CRITICAL: Route ALL "why/explain/how" questions here — even if they
        mention a specific CU ID like CU_COMMUNITY_003 or CU_ACU_001.
        The specificity comes from session data, NOT from re-running the engine.

        CRITICAL: Do NOT route to MATCHING_ENGINE for explain/why queries.
        Explain queries never re-run the pipeline.
