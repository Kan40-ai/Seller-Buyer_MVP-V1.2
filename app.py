...
parts_dict = {}
for (inst_id, as_of), group in parts_df.groupby(["institution_id", "as_of_date"]):  # type: ignore[misc]
...
def ltn_analyze():
    ...
    try:
        if (file.filename or "").lower().endswith(('.xls', '.xlsx')):
...
def _general_answer(user_query, history):
    # ── Greeting fast-path: never call LLM for a simple greeting ──────────────
    _GW = {"hi","hello","hey","morning","afternoon","evening","greetings","howdy","sup","hiya"}
    _cl = user_query.lower().strip("!.,? \n\t")
    _ws = set(_cl.split())
    if (_cl in _GW
            or _cl in {"good morning","good afternoon","good evening","good day"}
            or (len(_ws) <= 3 and bool(_ws & _GW))):
        persona   = session.get("persona", "analyst")
        cfg       = PERSONAS.get(persona, PERSONAS["analyst"])
        name      = session.get("username", "").title()
        sugg_html = "<br>".join(f"• <em>{s}</em>" for s in cfg.get("suggests", [])[:2])
        return (
            f"Hello{', ' + name if name else ''}! I'm your Portfolio Co-Pilot. 👋<br><br>"
            f"You're logged in as <strong>{cfg['label']}</strong>.<br><br>"
            f"Here's what you can try:<br>{sugg_html}"
        )
    # ──────────────────────────────────────────────────────────────────────────
    persona_tone     = session.get('persona_tone', 'helpful and professional')
    persona_label    = session.get('persona_label', 'user')
    ...
    messages = [
        {"role": "system", "content": (
            ...
            f"- LTV = Loan-to-Value ratio. DTI = Debt-to-Income ratio. FICO = credit score 300-850.\n\n"
            f"STRICT DATA RULES:\n"
            f"- NEVER fabricate, invent, or assume any metrics, figures, statistics, percentages, dollar amounts, or KPIs.\n"
            f"- NEVER generate placeholder numbers like '$150M', '250 requests', '12% increase' or any similar made-up data.\n"
            f"- Only reference real data explicitly present in an uploaded file or retrieved from NCUA.\n"
            f"- If no real data is available, say so clearly and ask the user to upload a file.\n"
            f"- For greetings or general queries, respond briefly and professionally — do NOT fabricate a dashboard summary.\n\n"
            f"FORMATTING RULES:\n"
            f"- Use <br> for line breaks. Use <strong>value</strong> for ALL key metrics.\n"
            f"- Use • for bullets. Never use markdown ** or ##. Never use markdown tables.\n"
            f"- Keep concise — max 3 lines per section.\n\n"
...
def chatbot_upload():
    ...
    # ── ADDED: Clear ALL stale session state on every new file upload ──────
    # This prevents boxing/matching state from previous operations bleeding
    # into the next route — root cause of LOAN_BOXING firing on call_report
    for key in [
        'chatbot_boxing_phase', 'chatbot_boxing_filters',
        'chatbot_boxing_original_name', 'last_boxing_csv_url',
        'last_refined_csv_url', 'raw_tape', 'final_box',
        'ltn_upload_path', 'matching_ran', 'chatbot_pending_file'
    ]:
        session.pop(key, None)
    # ───────────────────────────────────────────────────────────────────────
    ...
    # ── REVISED: Set new pending file AFTER clearing old state ────────────
    session['chatbot_pending_file'] = {
        "path": str(save_path),
        "name": original_name,
        "type": ftype
    }
...
def chatbot_message():
    ...
    # ══════════════════════════════════════════════════════════════
    # GREETING INTERCEPT — MUST BE FIRST — never reaches Groq
    # ══════════════════════════════════════════════════════════════
    _GREETING_WORDS = {
        "hi", "hello", "hey", "morning", "afternoon",
        "evening", "greetings", "howdy", "sup", "hiya"
    }
    _cleaned = user_msg.lower().strip("!.,? \n\t")
    _words   = set(_cleaned.split())

    is_greeting = (
        _cleaned in _GREETING_WORDS                          # exact: "hi", "hello"
        or _cleaned in {"good morning", "good afternoon",
                        "good evening", "good day"}          # exact two-word
        or (len(_words) <= 3 and bool(_words & _GREETING_WORDS))  # "hi there", "hey!"
    )
    ...
    if is_greeting:
        ...
        cfg          = PERSONAS.get(persona, PERSONAS["analyst"])
        name         = session.get("username", "").title()
        suggests     = cfg.get("suggests", [])
        suggest_html = "<br>".join(f"• <em>{s}</em>" for s in suggests[:2])
        answer = (
            f"Hello{', ' + name if name else ''}! I'm your Portfolio Co-Pilot. 👋<br><br>"
            f"You're logged in as <strong>{cfg['label']}</strong>.<br><br>"
            f"Here's what you can try:<br>{suggest_html}"
        )
    ...
    # explain_kw = [
    #     "how does", "explain", "why is", "why was", "why ranked",
    #     "how was", "what is the logic", "scoring logic", "methodology",
    #     "how are scores", "how do you rank", "how does it work",
    #     "what criteria", "how does the matching", "explain the matching",
    #     "explain the scoring", "explain these results", "explain the logic"
    # ]
    # if (session.get('matching_ran') or sbm.matching_state.get('final_ranking') is not None) \
    #     and any(kw in msg_lower for kw in explain_kw) \
    #     and not re.search(r'\bCU_[A-Z0-9_]+\b', user_msg, re.I):
    #     intent = "KNOWLEDGE_BASE"


    # # elif re.search(r'\bCU_[A-Z0-9_]+\b', user_msg, re.I):
    # #     intent = "MATCHING_ENGINE"
    
    # elif re.search(r'\bCU_[A-Z0-9_]+\b', user_msg, re.I) and not any(
    # kw in msg_lower for kw in [
    #     "why", "explain", "how", "what made", "what is",
    #     "tell me about", "deep dive", "breakdown", "score for",
    #     "why is", "why was", "why ranked", "reason"
    # ]
    # ):
    #     intent = "MATCHING_ENGINE"
    ...
    _explain_kw = [
        "why", "explain", "how does", "what made", "reasoning",
        "why is", "why was", "why ranked", "reason", "tell me about",
        "deep dive", "breakdown", "score for", "what is the logic",
        "how does", "scoring logic", "methodology", "how are scores",
        "how do you rank", "how does it work", "what criteria",
        "explain the matching", "explain the scoring", "explain these results"
    ]
    _has_cu_id        = bool(re.search(r'\bCU_[A-Z0-9_]+\b', user_msg, re.I))
    _has_explain      = any(kw in msg_lower for kw in _explain_kw)
    _has_matching     = (
        isinstance(sbm.matching_state.get('final_ranking'), pd.DataFrame)
        and not sbm.matching_state['final_ranking'].empty
    )

    # ── FIRST — explain intercept (beats everything including CU_ regex) ──
    if _has_explain and (_has_cu_id or _has_matching or session.get('matching_ran')):
        intent = "KNOWLEDGE_BASE"

    # ── CU_ ID without explain → run engine ──────────────────────────────
    elif _has_cu_id and not _has_explain:
        intent = "MATCHING_ENGINE"

    elif session.get('matching_ran') and any(kw in msg_lower for kw in [
        "generate the report", "generate report", "download report", "yes", "sure", "ok"
    ]):
        intent = "MATCHING_ENGINE"
    ...
    # =========================================================================
        ...
        if is_explain and not is_rerun:
            explain_summary = session.get('summary', {})
            print(f"[DEBUG EXPLAIN] summary available = {explain_summary}")

            if explain_summary:
                real_context = (
                    f"A loan boxing run was just completed in this session with these REAL results:\n"
                    f"- Total loans in tape: {int(explain_summary.get('total_loans', 0)):,}\n"
                    f"- Matched loans (in-box): {int(explain_summary.get('matched_loans', 0)):,}\n"
                    f"- Match rate: {float(explain_summary.get('match_pct', 0)):.1f}%\n"
                    f"- Weighted avg FICO: {float(explain_summary.get('wa_fico', 0)):.1f}\n"
                    f"- Weighted avg LTV: {float(explain_summary.get('wa_ltv', 0)):.1f}%\n"
                    f"- Weighted avg DTI: {float(explain_summary.get('wa_dti', 0)):.1f}%\n"
                    f"- Weighted avg Term: {float(explain_summary.get('wa_term', 0)):.1f} months\n"
                    f"- Matched balance: ${float(explain_summary.get('matched_balance_mm', 0)):.2f}M\n\n"
                    f"STRICT RULE: Reference ONLY these real figures in your explanation. "
                    f"Never fabricate or assume any other numbers."
                )
            else:
                real_context = (
                    "No boxing run has been performed yet in this session. "
                    "Explain the methodology conceptually and tell the user to "
                    "upload a loan tape first to see real analysis figures."
                )
            ...
            answer = _general_answer(
                ...
                f"{real_context}\n\n"
                f"Explain the Capital Flow AI loan boxing logic covering: "
                f"column standardisation, preprocessing pipeline, box definitions "
                f"(FICO/LTV/DTI bands), in_box_flag scoring logic, and what the "
                f"results above mean for this specific tape. "
                f"Speak as a {cfg['label']} would expect — tone: {cfg['tone']}.",
                history
            )
            history.append({"user": user_msg, "assistant": answer})
            session['chatbot_history'] = history[-10:]
            ...
            if has_filter_values and not skip_to_report:
                ...
                try:
                    with app.test_request_context('/ltn-analyze', method='POST', data=form_data):
                        ...
                        _r = ltn_analyze()
                        response = make_response(_r[0] if isinstance(_r, tuple) else _r)
                        raw_tape_r  = session.get('raw_tape')
            ...
            try:
                with app.test_request_context('/ltn-analyze', method='POST', data=saved_filters):
                    ...
                    if 'filename=' in content_disp:
                        html_filename = content_disp.split('filename=')[-1].strip('"').strip()
                    else:
                        html_filename = 'LTN_Report.html'
    ...
    #     # ── Inject real matching state if available ───────────────────
    #     matching_context = ""
    #     if sbm.matching_state.get('final_ranking') is not None:
    #         liquidity     = sbm.matching_state.get('liquidity', {})
    #         final_ranking = sbm.matching_state['final_ranking']
            
    #         matching_context = (
    #             f"A matching run was completed in this session:\n"
    #             f"- CU analysed: {liquidity.get('label', 'N/A')}\n"
    #             f"- LTD: {liquidity.get('ltd', 0):.1%}\n"
    #             f"- Role: {liquidity.get('role', 'N/A')}\n"
    #         )
    #         if not final_ranking.empty:
    #             matching_context += "- Top counterparty rankings:\n"
    #             for i, (_, row) in enumerate(final_ranking.head(3).iterrows(), 1):
    #                 matching_context += (
    #                     f"  #{i} {row['seller_id']} — "
    #                     f"Final Score: {row['final_score']:.0f} "
    #                     f"(Activity: {row.get('activity_score', 0):.0f}, "
    #                     f"Capacity: {row.get('capacity_score', 0):.0f})\n"
    #                 )

    #     query_with_context = (
    #         f"The user asked: '{user_msg}'.\n\n"
    #         f"{matching_context}\n"
    #         f"STRICT RULE: Reference ONLY the real scores and CU IDs above. "
    #         f"Never fabricate scores or rankings not listed above."
    #         if matching_context else user_msg
    #     )
    # # ─────────────────────────────────────────────────────────────
    ...
    # =========================================================================
        ...
        # ── Debug ─────────────────────────────────────────────────────────────
        print(f"[DEBUG KB] matching_state keys = {list(sbm.matching_state.keys())}")
        final_r = sbm.matching_state.get('final_ranking')
        print(f"[DEBUG KB] final_ranking = {type(final_r)} | empty = {final_r is None or (hasattr(final_r, 'empty') and final_r.empty)}")
        print(f"[DEBUG KB] liquidity = {sbm.matching_state.get('liquidity', {})}")
        # ─────────────────────────────────────────────────────────────────────
        ...
        if sbm.matching_state.get('final_ranking') is not None:
            ...
            if not final_ranking.empty:
                ...
                f"- LTD: {liquidity.get('ltd', 0):.1%}\n"
                f"- Role: {liquidity.get('role', 'N/A')}\n"
                f"- Top counterparty rankings (ALL scores are real — do not invent any):\n"
            )
                for i, (_, row) in enumerate(final_ranking.head(3).iterrows(), 1):
                    ltd_bonus  = row.get('ltd_bonus', row.get('ltd_bonus_act', 0) or 0)
                    size_bonus = row.get('size_bonus', row.get('size_bonus_act', 0) or 0)
                    geo_bonus  = row.get('geo_bonus', row.get('geo_bonus_act', 0) or 0)
                    matching_context += (
                        f"  #{i} {row['seller_id']}:\n"
                        f"     Activity Score:  {row.get('activity_score', 0):.0f}\n"
                        f"     Capacity Score:  {row.get('capacity_score', 0):.0f}\n"
                        f"     LTD Band Bonus:  +{ltd_bonus:.0f} pts\n"
                        f"     Size Band Bonus: +{size_bonus:.0f} pts\n"
                        f"     Geo Bonus:       +{geo_bonus:.0f} pts\n"
                        f"     FINAL SCORE:     {row['final_score']:.0f}\n"
                    )

                # Also add score formula from engine
                matching_context += (
                    f"\nActual scoring formula:\n"
                    f"Final Score = Activity Score + Capacity Score + LTD Bonus + Size Bonus + Geo Bonus\n"
                    f"(No percentage weights — it is a direct sum of all components)\n"
                    f"STRICT: Never invent weights like 0.6x or 0.2x — the engine uses direct addition only."
                )



                print(f"[DEBUG KB] matching_context built = {matching_context}")
        # ─────────────────────────────────────────────────────────────────────
        ...
        if pending and pending['type'] == 'rag_doc':
            ...
            # ── Direct Groq call with real matching data ───────────────────
            try:
                messages = [
                    {"role": "system", "content": (
                        f"You are Portfolio Co-Pilot for Capital Flow AI.\n"
                        f"STRICT RULE: Use ONLY the real scores and CU IDs provided "
                        f"below. Never invent percentage weights, scoring formulas, "
                        f"or figures not explicitly listed here.\n\n"
                        f"Real matching session data:\n{matching_context}"
                    )},
                    {"role": "user", "content": (
                        f"{user_msg}\n\n"
                        f"Explain using the real CU IDs and scores above. "
                        f"Cover Activity score, Capacity score, LTD band bonus, "
                        f"Size band bonus and Geo bonus — not generic FICO/LTV weights."
                    )}
                ]
                ...
                answer = _clean_markdown(resp.choices[0].message.content.strip())
            except Exception as e:
                answer = f"Response error: {e}"
            # ─────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("PORT", 0)) or _free_port([5000, 5001])
